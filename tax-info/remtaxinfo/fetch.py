...
async def update_tax_info(prop: Property, mongo_db: Any, http_session: Any) -> None:
    ...
    for pin_number in prop.pin_numbers:
        ...
        tasks.append(
            update_with_adapter(prop.id, pin_number, adapter_inst, mongo_db, http_session, task_logger)
        )
...
async def update_with_adapter(property_id, pin, adapter_inst, mongo_db, http_session, task_logger):
    """
    ...
    async def update_pin_tax_bills_for_year(pin, year, task_logger):
        ...
        """
        try:
            tax_bill = await adapter_inst.get_tax_bill(pin, year, http_session)
            await update_property(
