...
from remcommon.models_gen import Property, TaxPropInfo
...
async def update_tax_info(prop: Property, mongo_db: Any, http_session: Any) -> None:
    """
    ...
    for pin_number in prop.pin_numbers:
        tasks.append(update_with_adapter(prop.id, pin_number, adapter_inst, mongo_db, http_session))
...
async def update_with_adapter(property_id, pin, adapter_inst, mongo_db, http_session):
    """
    ...
    task_logger = FieldLogger(
        logger, {"county": adapter_inst.county, "state": adapter_inst.state, "property_id": property_id}
    )

    async def update_property(update_obj: Dict):
        await mongo_db.properties.update_one({"_id": property_id}, update_obj)
    ...
    async def update_pin_property_info(pin, task_logger):
        ...
        """
        try:
            prop_info = await adapter_inst.get_latest_property_info(pin, http_session)
    ...
    async def update_pin_tax_bills_for_year(pin, year, task_logger):
        ...
        """
        try:
            tax_info = await adapter_inst.get_tax_bill(pin, year, http_session)
            await update_property(
                {"$set": {f"{fields.PROPERTY_TAX_BILLS}.{pin}.{year}": TaxPropInfo.to_dict(tax_info)}}
            )
...
async def remove_stale_pin_info(prop: Property, mongo_db):
    """
    Take out property information for pins that are no longer associated with the property
    """
    if prop.tax_prop_info is None:
        return

    for pin in prop.tax_prop_info.keys():
