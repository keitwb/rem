...
import { ModelUpdate, patchWithObject } from "@/model/updates";
...
// Axios request transformer to add an etag
function addEtag(etag: ETag = null): (data: any, headers: any) => any {
  return (data, headers) => {
    if (etag) {
      headers["If-Match"] = etag.$oid;
    }
    return data;
  };
}
...
export class MongoClient {
  ...
  public fetchNew<T extends MongoDoc>(res: axios.AxiosResponse<T>): Promise<ValOrErr<T>> {
    if (res.status !== 201) {
  ...
  public async create<T extends MongoDoc>(collection: string, obj: object): Promise<ValOrErr<MongoID>> {
  ...
  private async createFile<T extends MongoDoc>(collection: string, file: File, metadata: object): Promise<ValOrErr<T>> {
